...
evaluate/
├── tasks.md          # 标准任务集（训练用）
├── tasks-v2.md       # 进阶任务集（v1 全部 >90 后使用）
├── rubrics.md        # 评分标准（所有角色的 rubric 放一起）
└── GUIDE.md          # 本指南
```

新增角色时：
1. 在 `evaluate/tasks.md` 的末尾添加 `## <角色> Tasks` 段落
2. 在 `evaluate/rubrics.md` 的 `---` 分隔符后添加 `## <角色> Rubric` 段落
3. 创建 `skills/<角色>/SKILL.md` 作为初始技能定义
